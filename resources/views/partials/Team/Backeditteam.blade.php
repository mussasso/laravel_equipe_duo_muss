<section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800 my-6">
    ...
    <form action="/team/{{$team->id}}" method="post" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input id="" type="number" name="maxplayers" value="{{old('maxplayers')}}" class="my-5 block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
        </div>

        <div>
            <select name="continent_id">
                @foreach ($continent as $continents)
                    <option value="{{$continents->id}}">{{$continents->continent}}</option>
                @endforeach
            </select>
        </div>
    </div>
